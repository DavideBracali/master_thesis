...
\begin{itemize}
    \item Define the most discriminative local features to differentiate
    between contacts that were labeled as SOZ and healthy regions of the brain;
    \item Illustrate a classification example for one representative
    patient and interpret intermediate results in the pipeline.
\end{itemize}
...
example on a representative patient from the analyzed subset.
This case study presents the training and anomaly detection process in the
latent space, and shows how local feature deviations translate into alarm scores.  
To avoid overloading the reader with an excessive amount of figures, only one
representative patient will be shown.
(!!! fare anche appendice).
...
shows that moderate number of low-likelihood outliers is present in the training 
...
\begin{figure}[H]
    ...
    \caption{VAE encoding of the seizure onset on GMM fit of resting-state feaures.}
...
alone cannot detemine which contacts contribute to the seizure onset, but its
...
a few seconds before the generalized seizure, identifiable by the simultaneus
and widespread anomalies.
Gaussian smoothing reduces noise while preserving prolonged anomalies,
mantaining meaninfgul information while discarding isolated peaks.
...
\begin{figure}[H]
    ...
    \caption{!!! Come mai non è centrato? Cambiare immagine, mostrare magari i top 100}
...
To limit computation time, the VAE and GMM were trained on an uniformly sampled 
subset of 200 seconds of temporal observations drawn between the training
patients.
...
each LOOCV fold.
Preliminarily to the classifier optimization, VAE and GMM are trained on an 
uniformly sampled subset of 2000 seconds of temporal observations drawn between 
...
This result is likely to be a consequence the of combination of two factors:
...
\ref{fig:exDE:tr} in Section \ref{res:anal:example}).
...
sampling from a normal distribution whose mean and log-variance that are built
from a linear combination of the resting-state feautures.
The reparametrization trick still introduces non-linearity, but the best
encoder model is essentially a linear perceptron.
...
The dataset proved to be very diverse, while a moderately low number of 
patients was available for this study.
As a consequence, all metrics reported in Section \ref{res:eval} highlight that
the model struggles to find an unique set of parameters to generalize to many 
...
The current implementation does not not fully exploit its non-linear feature
...
The decision to preform a two-step hyperparameter optimization is also a
limitation itself.
...
final results heavily dependend on the choice of the intermediate target. 
