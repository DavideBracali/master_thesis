...
We will also define index $n=0,1,...,F$ so that $|Z_n|^2$ is the power (squared
amplitude) of frequency $f_n$ in the Fourier transform.
...
\begin{itemize}
    ...
    \begin{equation}
        S_\text{flatness}=
        \frac{\exp\left(\frac{1}{F+1}\sum_{n=0}^{F}\ln |Z_n(\mathbf{x})|^2\right)}
        {\frac{1}{F+1}\sum_{n=0}^{F}|Z_n(\mathbf{x})|^2}
    \end{equation}
    ...
    1. Formally, the relative powerband between frequencies $a$ and $b$ is
    defined as:
    \begin{equation}
    S_{\text{band}} = \frac{1}{C}
    \sum\limits_{f_n \in [a, b]} |Z_n(\mathbf{x})|^2
    \end{equation}
    where $\sum\limits_{n=0}^{F} |Z_n(\mathbf{x})|^2$ is a normalization factor
    forcing the sum of all powerbands to be equal to 1.
    In this study we compute the following relative powerbands:
\begin{table}[h]
\centering
\begin{tabular}{lcc}
\hline
