...
\begin{itemize}
    ...
    \item The number of hidden layers $n_{\text{hidden}}$.
...
To decide which contact consitute the SOZ, we use a deterministic classifier
...
This operation is preformed to remove noise from the time-series and to 
...
\begin{itemize}
    ...
    \begin{equation}
        b_c(t) = \left|\frac{b_c(t)-\mu_{\text{train},c}}
...
\begin{equation}
    ...
    \left( \alpha\, f_{t_C}(t) + (1-\alpha)\, b_c(t) \right) \text{sNLL}_c(t)
\end{equation}
...
Each fold is composed on 18 training patients, and one validation patient.
...
architecture.
This step depends on 6 hyperparameters: $\eta$, $n_{\text{hidden}}$, $d$,
...
\ref{meth:classifier}) will use the negative-log likelihood ($NNL$) to compute the
alarm score.
...
𝜏
τ as an additional hyperparameter to be sampled during optimization, we determine it deterministically after training by selecting the value that maximizes the median performance across patients.
