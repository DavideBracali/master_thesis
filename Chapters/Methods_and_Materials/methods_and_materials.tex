...
distributions, and is particularly efficient in high dimensionality spaces
...
We therefore use, for each seizure vector $\mathbf{x}_i$ the negative
log-likelihood as a quantitative indicator for anomalous behavior with respect
to the resting-state:
\begin{equation}
    -\log_{10}\mathcal{L}(\mathbf{\psi|x_i})    
\end{equation}
where $\psi$ represents the GMM parameters.
...
!!! Descrivi il classificatore: 
smoothing, peso temporale, peso betweenness, coordinate spaziali, thresholding

\subsection{Hyperparameter Tuning}  \label{meth:opt}

!!! Descrivi ottimizzazione iperparametri: cross-validation, 2 fasi, target,
spiegare perché non ci interessano i falsi negativi
